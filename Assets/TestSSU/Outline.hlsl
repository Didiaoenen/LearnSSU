...
VertexOutput OutlineVertex(VertexInput v)
{
    ...
    float3 _BakedNormalDir = normalize(mul(_BakedNormal_var.xyz, tangentTransform));
    //end
    
    float4 objPos = mul(unity_ObjectToWorld, float4(0, 0, 0, 1));
    float Set_Outline_Width = (_Outline_Width * 0.001 * smoothstep(_Farthest_Distance, _Nearest_Distance, distance(objPos.xyz, _WorldSpaceCameraPos)) * _Outline_Sampler_var.rgb).r;
...
#elif defined(_OUTLINE_POS)
    Set_Outline_Width = Set_Outline_Width * 2;
    float signVar = dot(normalize(v.vertex.xyz), normalize(v.normal)) < 0 ? -1 : 1;
    o.pos = UnityObjectToClipPos(float4(v.vertex.xyz + signVar * normalize(v.vertex.xyz) * Set_Outline_Width, 1));
#endif
...
float4 OutlineFragment(VertexOutput i) : SV_Target
{
    //v.2.0.5
